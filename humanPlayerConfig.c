...
bool gameOver(char computer, char player, char board[][26], int n, bool invalidPlay){
int compP = 0, playP = 0;
...
// Iterate through each square 
  for(int rows =0; rows<n; rows++){
    for(int cols = 0; cols<n; cols++){
      if (board[rows][cols] == computer){compP++;}
      else if (board[rows][cols] == player){playP++;}
    }
  }
...
void printBoard(char board[][26], int n) { // Print board as it is @ time of use 
...
bool positionInBounds(int n, int row, int col) { // Check if (row, col) input is within the board's dimensions
  bool isIt = true;
  if( row < n && col < n && row >= 0 && col >= 0){ isIt = true;}
  else{isIt = false;}
  return isIt; }

// check if some input (row , col) is a legal move = inside bounds and has the stupid opp colour and our colour 
bool checkLegalInDirection(char board[][26], int n, int row, int col, char colour, int deltaRow, int deltaCol) { 
  ...
  for(int i = 1; exit == false && legality == false; i++){
    ...
    // Otherwise
      ...
      if(board[row + (deltaRow*i)][col + (deltaCol*i)] == opp){count++;}
      ...
      // Letter is not the opposing colour, so, letter is U or our colour
        ...
        if(count != 0  && board[row + (deltaRow*i)][col + (deltaCol*i)] == colour){ legality = true;}
        // Letter is U 
        else{ exit = true;}
      }
    }
...
bool hasAvailableMoves(char board[][26], int n, char colour){ // finds a (row, col) at which we can place = U --> legaldir
...
bool inputValidation(char board[][26], int n, char colour, int row, int col){ // Checks player input shit 
  ...
  for(int i = -1; i<=1 && foundOne == false; i++){
    ...
    if (foundOne ==false){  //In bounds play, but not a legal move in terms of gameplay
      validation = false;
  }
}
return validation;
...
void flipTile(char board[][26], int n, char colour, int row, int col){ //Flips ALL tiles at input (row, col)
    ...
    for(int j = -1; j<=1; j++){
      if(!(i ==0 && j == 0)){
        ...
        if(checkLegalInDirection(board,n,row, col, colour,i, j)){
          ...
          // Flip the preceeding tiles until you hit your own tile in that direciton
          for(int tile = 1; board[row + (i*tile)][col + (j*tile)]!=colour; tile++){
...
void boardConfiguration(char board[][26]){ // Midgame functionality 
  ...
  while(!(colourC == '!' && rowC == '!' && columnC == '!')){ // Iterate as long as input is not '!!!'
    indR = rowC - 'a';
    indC = columnC - 'a';

    if(colourC == 'B'){board[indR][indC] = 'B';}
    else{board[indR][indC] = 'W';}
  ...
  for(int rows =0; rows<n; rows++){
    for(int cols = 0; cols<n; cols++){
      ...
      if(board[rows][cols] == 'U'){ //If it is unoccupied:
        ...
        //bool foundOne = false;  // did NOT find square of own colour
        ...
        for(int i = -1; i<=1 ; i++){
          for(int j = -1; j<=1 ; j++){
            if(!(i ==0 && j == 0)){
              ...
              if(checkLegalInDirection(board,n,rows, cols, currentPlay,i, j)){ // Check (rows, cols) is legal

                // Sum the preceeding tiles until you hit your own tile in that ONE direciton
                for(int tile = 1; board[rows + (i*tile)][cols + (j*tile)]!=currentPlay; tile++){
                  score++;
                }
              }
            }
        ...
        if(score>Hscore){ // If the most tiles are flipped --> make this the play for comp 
          Hscore = score;
          Hrow = rows;
          Hcol = cols;
          score = 0;  //Reset score
...
bool inputPlay(char currentPlay, char computer, char player, char board[][26], int n){
    ...
    if(currentPlay == computer){    // computer play
      ...
      printBoard(board, n);
        ...
        else if( inputValidation(board, n,  colour ,row -'a', column - 'a') == false){
          printf("Invalid move.\n");
...
int main(void) {
  ...
  for (int rows = 0; rows < n; rows++) {
    ...
    int b = n-1;
      if ((rows == (b/2) && cols == (b/2)) ||  (rows == ((b/2)+1) && cols ==((b/2)+1))) {
        board[rows][cols] = 'W';
      }

      else if ((rows == (b/2) && cols == ((b/2)+1)) || (rows == ((b/2)+1) && cols == (b/2)) ) {
        board[rows][cols] = 'B';
      }

      else{board[rows][cols] = 'U'; }
...
//First play --> always black player
    isGameOver = inputPlay('B', computer, player, board, n);   //Pass currentPlay as B
